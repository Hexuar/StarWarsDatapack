$summon marker ~ ~ ~ {CustomName:'{"text":"Blaster Bolt"}',Tags:["starwarspack.entity","starwarspack.blaster_bolt","starwarspack.new"],data:$(bolt)}
...
$execute if score #1 starwarspack.const matches $(explode_on_impact) run tag @e[type=marker,tag=starwarspack.new,sort=nearest,limit=1,distance=..4] add starwarspack.explode_on_impact
